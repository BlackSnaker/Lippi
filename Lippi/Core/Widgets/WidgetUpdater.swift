...
enum WidgetUpdater {
    ...
    private static let updateQueue = DispatchQueue(label: "WidgetUpdater.io", qos: .utility)
    ...
    static func updatePomodoro(phase: PomodoroPhase, start: Date?, end: Date?, round: Int) {
        let phaseValue = phase.rawValue
        updateQueue.async {
            defaults?.set(phaseValue, forKey: WidgetShared.pomodoroPhaseKey)
